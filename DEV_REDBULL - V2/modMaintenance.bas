...
' Fonction pour optimiser la base de données (si autorisé)
Public Sub OptimiserBDD()
    ...
    Dim reponse As Integer
    reponse = MsgBox("Lancer l'optimisation de la base de données ?" & vbCrLf & _
                     "Cette opération peut prendre plusieurs minutes.", _
                     vbYesNo + vbQuestion, "Optimisation BDD")
    
    If reponse = vbYes Then
        ' Exemple de commandes d'optimisation (à adapter selon votre SGBD)
        ' ATTENTION : Ces commandes peuvent être risquées en production
        
        MsgBox "Optimisation BDD simulée" & vbCrLf & _
               "Dans un environnement réel, cette fonction exécuterait :" & vbCrLf & _
               "• UPDATE STATISTICS" & vbCrLf & _
               "• REINDEX des tables principales" & vbCrLf & _
               "• VACUUM (PostgreSQL) ou SHRINK (SQL Server)", _
               vbInformation, "Optimisation"
    End If
