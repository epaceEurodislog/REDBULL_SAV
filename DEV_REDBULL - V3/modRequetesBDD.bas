...
Public Function RecupererNumeroReception(numeroSerie As String) As TypeDonneesREE
    ...
    If Not rs.EOF Then
        If Not IsNull(rs!REE_NORE) And Trim(rs!REE_NORE) <> "" Then
            With resultats
                .trouve = True
                .numeroReception = Trim(rs!REE_NORE)
                .numeroEnlevement = ""
                .messageErreur = ""
            End With
        Else
            resultats.messageErreur = "REE_NORE existe mais est vide"
        End If
    Else
        resultats.messageErreur = "Aucun enregistrement trouvé"
    End If
...
Public Sub DiagnostiquerProbleme(numeroSerie As String)
    ' Test 1 : Vérifier la connexion
    Debug.Print "Connexion : " & conn.connectionString
    
    ' Test 2 : Test simple
    Dim sql As String
    sql = "SELECT COUNT(*) as total FROM NSE_DAT WHERE NSE_NUMS = '" & numeroSerie & "'"
    Dim rs As ADODB.Recordset
    Set rs = New ADODB.Recordset
    rs.Open sql, conn
    Debug.Print "NSE_DAT count : " & rs!total
    rs.Close
    
    ' Test 3 : Vérifier les types
    sql = "SELECT STK_NoSU, ART_CODE FROM NSE_DAT WHERE NSE_NUMS = '" & numeroSerie & "'"
    rs.Open sql, conn
    If Not rs.EOF Then
        Debug.Print "STK_NoSU type : " & TypeName(rs!STK_NoSU)
        Debug.Print "STK_NoSU valeur : [" & rs!STK_NoSU & "]"
    End If
    rs.Close
    
    ' Test 4 : Vérifier REL_DAT
    sql = "SELECT REL_NoSU FROM REL_DAT WHERE ACT_CODE = 'RB' AND REL_NoSU IS NOT NULL"
    rs.Open sql, conn
    Debug.Print "Premier REL_NoSU trouvé : " & rs!REL_NoSU
    Debug.Print "REL_NoSU type : " & TypeName(rs!REL_NoSU)
    rs.Close
End Sub
